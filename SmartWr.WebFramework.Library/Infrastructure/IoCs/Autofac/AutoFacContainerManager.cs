...
namespace SmartWr.WebFramework.Library.Infrastructure.IoCs.Autofac
{
    public class AutoFacContainerManager : IContainerManager
    {
        ...
        public void AddControllerFromAssembly(Assembly assembly, ComponentLifeStyle lifeStyle = ComponentLifeStyle.Singleton)
        {
            UpdateContainer(x =>
            {
                x.RegisterApiControllers(assembly).PerLifeStyle(lifeStyle);
                x.RegisterControllers(assembly).PerLifeStyle(lifeStyle);
            });
        ...
        public static IRegistrationBuilder<TLimit, TActivatorData, TRegistrationStyle> PerLifeStyle<TLimit, TActivatorData, TRegistrationStyle>(this IRegistrationBuilder<TLimit, TActivatorData, TRegistrationStyle> builder, ComponentLifeStyle lifeStyle)
        {
            switch (lifeStyle)
            {
                case ComponentLifeStyle.LifetimeScope:
                    //return HttpContext.Current != null ? builder.InstancePerHtpRequest() : builder.InstancePerLifetimeScope();
                    return HttpContext.Current != null ? builder.InstancePerMatchingLifetimeScope() : builder.InstancePerLifetimeScope();
